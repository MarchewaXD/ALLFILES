...
<body>
    ...
    <main id="system-index">
        <nav>
            <section class="menuC"><h2>Faktury Kosztowe</h2></section>
            <a href="sprzedazowe.php"><section class="menu"><h2>Faktury Sprzedażowe</h2></section></a>
        </nav>
...
if ($numer_faktury != "" && $data_wystawienia != "" && $kontrahent_id != "" && $wartosc_brutto != "" && $czy_oplacona != "") {
    ...
    // zapytanie SQL
    $sql = "INSERT INTO kosztowe (numer_faktury, data_wystawienia, kontrahent_id, wartosc_brutto, oplacona)
            VALUES ('$numer_faktury', '$data_wystawienia', $kontrahent_id, $wartosc_brutto, $czy_oplacona)";

    // wykonanie zapytania
    if (mysqli_query($conn, $sql)) {
        echo "<p>Dodano fakturę!</p>";
        echo '<meta http-equiv="refresh" content="3;url=kosztowe.php">';
    } else {
